...
export default class Desktop {
    ...
    constructor({
        ...
        const fhirDancer = new Module('fhirDancer');
        ...
        eventsBus.on('desktopLoaded', (window) => {
            console.log('desktopLoaded')
        });
        eventsBus.on('loadingFinished', (window) => {
            console.log('loadingFinished')
        });
        eventsBus.on('afterInitialization', (window) => {
            console.log('afterInitialization')
        });

        

        fhirDancer.on('getAll', (event, fetchId) => {
            this.log.verbose('fhirDancer.getAll received');
            console.log('fhirDancer.getAll.received')

            fhirDancer.respond('getAll', fetchId, "abc");
            console.log('fhirDancer.getAll.responded')
        });
